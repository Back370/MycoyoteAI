import numpy as np
def update_strategy_network(strategy_net, advantage_net, reservoir_buffer, batch_size = 32, epochs = 12):
    """
    ...
    #policy_targets = np.zeros((batch_size, strategy_net.output_size), dtype=float) # Changed to (32, 141) and dtype=float
    policy_targets = []
    ...
    for i in range(len(states)):
      ...
      if sum_positive > 0:
          policy = positive_advantages / sum_positive
      else:
          # Uniform strategy over legal actions if no positive regret
          policy = np.zeros_like(positive_advantages)
          for action in legal_actions:
              policy[action] = 1.0 / len(legal_actions)

      policy_targets.append(np.argmax(policy)) # Get the index of the maximum probability
