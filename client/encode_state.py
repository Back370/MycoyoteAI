...
def get_current_state_from_rounds(round_info: List[Dict[str, Any]], round_idx: int = -1, player_name: str = 'r') -> Dict[str, Any]:
    """
    ...
    # 合計値を計算
    sum_val = sum(player.get("card", 0) for player in players if isinstance(player.get("card", 0), (int, float)))
...
def encode_state(state: Union[Dict[str, Any], str]) -> np.ndarray:
    ...
    # 辞書からキーを取得
    if isinstance(state, str):
        raise ValueError("Expected 'state' to be a dictionary, but got a string.")
    ...
    #assert len(features) == 1000, f"Feature vector size is incorrect: {len(features)} != 1000"  # Ensure correct input siz
