...
from encode_state import encode_state, encode_batch_states
...
def train_deepcfr_for_coyote(iterations=1000, current_state = None):
    ...
    """
    num_players = 6
    # それぞれのプレイヤーにアドバンテージネットワークと戦略ネットワークを作成
    ...
    try:
      ...
      # 状態をエンコード
      encoded_state = encode_state(current_state) # type: ignore
    ...
    for i in range(iterations):
        ...
        if i % 10 == 0:
            for player in range(num_players):
                strategy_nets[player].model.save(f"C:/Users/PCUSER/VScode-project/resource/models/model_{player}.keras")
